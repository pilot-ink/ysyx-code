...
void init_iringbuf(){
    rbuf = (iringbuf *)malloc(sizeof(iringbuf));
    for(int i = 0; i < Ringbuffer_max;i++)
        rbuf->buffer[i] = (char *)malloc(sizeof(char) * 128);
    rbuf->end = 0;
    rbuf->start = 0;
}
...
void print_iringbuf(){
    for(int i = rbuf->end;(i%Ringbuffer_max) != rbuf->start;i=(i+1)%Ringbuffer_max)
     printf("inst:%s\n",rbuf->buffer[i]);
}
#endif
...
void read_elf(char *file){
    ...
    //elf header and 读取文件
    Elf32_Ehdr *read_elf_file = (Elf32_Ehdr *)malloc(sizeof(Elf32_Ehdr));
    char *pbuff = (char *)malloc(get_file_size(fp));
    char *buffer = pbuff;
    fread(pbuff, sizeof(char), get_file_size(fp), fp);
    //链表，用于保存ftrace需要遍历的函数信息
    head_st = (struct linked_list *)malloc(sizeof(struct linked_list));
    ptr_list_tail = (struct linked_list *)malloc(sizeof(struct linked_list));;
    ptr_list_tail->next = head_st;

    //Magic
    strncpy((char *)read_elf_file->e_ident,pbuff,EI_NIDENT);
    pbuff += EI_NIDENT;
    //Type
...
void init_fringbuf(){
    fbuf = (fringbuf *)malloc(sizeof(fringbuf));
    for(int i = 0; i < fRingbuffer_max;i++)
        fbuf->buffer[i] = (func_info *)malloc(sizeof(char) * 128);
}
...
char *find_linked_list(vaddr_t pc){
    ...
    if(pc == 0x8000000C) return "_start";
    return NULL;
}
    ...
    for(int i = 0;i<ncount;++i)
    {
        ...
        switch(typelow)
        {
            ...
            case STT_OBJECT:
                break;
            case STT_FUNC:
                ptr_list_tail->next->next = (struct linked_list *)malloc(sizeof(struct linked_list));
...
mringbuf *mbuf;
void init_mringbuf(){
    mbuf = (mringbuf *)malloc(sizeof(mringbuf)); 
    mbuf->start = 0;
    mbuf->end = 0;
}
