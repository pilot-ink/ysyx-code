...
#define immB() do { *imm = (SEXT((BITS(i, 31, 31) << 12), 12)) | (BITS(i, 30, 25) << 5) | (BITS(i, 7, 7) << 11) | (BITS(i, 11, 8) << 1); } while(0)
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  //INSTPAT("0000000 ????? ????? 001 ????? 00100 11", slli   , I, src1 += imm; R(rd) = src1); 
  //INSTPAT("0000000 ????? ????? 101 ????? 00100 11", srli   , I, src1 += imm; R(rd) = src1); 
  //INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I, src1 += imm; R(rd) = src1); 
  INSTPAT("??????? ????? ????? 010 ????? 00100 11", slti   , I, R(rd) = ((int)src1 < imm)?1:0); 
  ...
  INSTPAT("??????? ????? ????? 100 ????? 00000 11", lbu    , I, R(rd) = SEXT(0,32) | Mr(src1 + imm, 1)); //maybe
  INSTPAT("??????? ????? ????? 101 ????? 00000 11", lhu    , I, R(rd) = SEXT(0,32) | Mr(src1 + imm, 2));  //maybe
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 01100 11", sra    , R, R(rd) = (int)src1 >> src2);  //shift right arith
