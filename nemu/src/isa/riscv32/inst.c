...
#define immS() do { *imm = (SEXT(BITS(i, 31, 25) << 5, 12)) | BITS(i, 11, 7); } while(0)
#define immJ() do { *imm = (SEXT(BITS(i, 31, 31) << 20, 21)) | (BITS(i, 20, 20) << 11) | (BITS(i, 19, 12) << 12) | (BITS(i, 30, 21) << 1); } while(0)
#define immB() do { *imm = (SEXT((BITS(i, 31, 31) << 12), 1)) | (BITS(i, 30, 25) << 5) | (BITS(i, 7, 7) << 11) | (BITS(i, 11, 8) << 1); } while(0)
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0000000 ????? ????? 001 ????? 00100 11", slli   , I, R(rd) = src1 << (SEXT(1, 1) & imm)); //maybe
  INSTPAT("0000000 ????? ????? 101 ????? 00100 11", srli   , I, R(rd) = src1 >> (SEXT(1, 1) & imm)); //maybe
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I, R(rd) = (int)src1 >> (SEXT(1, 1) & imm)); //maybe
